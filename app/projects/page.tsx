...
export default function ProjectsPage() {
    ...
    const filteredMyProjects = myProjects.filter(
        (project) => project.status === myProjectTab
    );
                        ...
                        {/* <div className="mt-4 flex justify-center gap-2">
                            {[0, 1].map((index) => (
                                <button
                                    ...
                                    className={`h-2 rounded-full transition-all ${progressSlide === index ? "w-6 bg-white" : "w-2 bg-white/50"
                                        }`}
                                    aria-label={`Show progress slide ${index + 1}`}
                                />
                            ))}
                        </div> */}
                    </section>
